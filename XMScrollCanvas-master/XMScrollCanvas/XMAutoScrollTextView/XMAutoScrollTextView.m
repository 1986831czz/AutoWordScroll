...
-(void)createContentOfScrollView{
    ...
    for (int j=0; j<2;j++ ) {
        ...
        for (int i=0; i<self.textArray.count; i++) {
            UILabel *textLabel=[[UILabel alloc] initWithFrame:CGRectMake(self.contentSize.width, labelY, 0, labelH)];
            ...
            if (i<self.textArray.count) {
                ...
                [self fitStringLength:textLabel];
                labelW = [textLabel sizeThatFits:CGSizeMake(MAXFLOAT, labelH)].width + 100;
                CGRect frame = textLabel.frame;
                frame.size.width = labelW;
...
- (void)fitStringLength:(UILabel *)textLabel {
    //    CGFloat wordLength = [textLabel sizeThatFits:CGSizeMake(MAXFLOAT, textLabel.layout_height)].width;
    while ([textLabel sizeThatFits:CGSizeMake(MAXFLOAT, textLabel.frame.size.height)].width < self.frame.size.width) {
...
- (void)autoScroll{
    ...
    if (self.contentOffset.x>=self.contentSize.width/2) {
        self.contentOffset=CGPointMake(self.recordOffSetX - self.contentSize.width/2, 0);
    }
    ...
    [UIView animateWithDuration:SCROLL_TIME_INTERVAL delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        self.recordOffSetX = self.contentOffset.x;
    }];
}

//- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
//    CGFloat offSetX = scrollView.contentOffset.x;
//    if (offSetX >= self.contentSize.width/2 ) {
//      self.contentOffset=CGPointMake(0, 0);
//    }
//}
@end
